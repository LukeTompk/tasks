...
export function sameType(questions: Question[]): boolean {
    ...
    const initType = questions[0].type;
    return questions.every((question) => question.type === initType);
}
...
export function changeQuestionTypeById(
    ...
    return questions.map((question) => {
        ...
        if (updatedQuestion.id === targetId) {
            updatedQuestion.type = newQuestionType;
            if (updatedQuestion.type !== "multiple_choice_question") {
...
export function duplicateQuestionInArray(
    ...
    for (const question of questions) {
        ...
        if (question.id === targetId) {
            newArray.push(duplicateQuestion(newId, question));
        }
    }
    return newArray;
}
